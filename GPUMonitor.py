...
class GPUData:
    def __init__(self, index, name, vram_total_mb):
        self.index      = index
        self.name       = name          # full NVML name, no stripping
...
class SystemData:
    ...
    def __init__(self):
        self.cpu_pct  = 0.0
        self.cpu_temp = None   # may be None if platform doesn't support it
        self.ram_pct  = 0.0
        self.ram_used_mb = 0
        self.ram_total_mb = 0
        self.h_cpu  = []
        ...
        if PSUTIL_OK:
            ...
            # prime cpu_percent so first real call returns a value
    ...
    def ingest(self):
        ...
        # CPU temp — works on Windows via WMI sensors
        try:
...
class DiskData:
    ...
    def __init__(self):
        self.disks   = {}   # name -> dict of current values
        self.h_read  = {}   # name -> list of MB/s
        self.h_write = {}   # name -> list of MB/s
        self.h_active= {}   # name -> list of active %
        self._prev   = {}   # name -> (bytes_read, bytes_write, busy_ms, timestamp)
        if PSUTIL_OK:
            self._init_disks()
    ...
    def _init_disks(self):
        try:
            ...
            for name, c in counters.items():
                # Filter out optical drives / loop devices (common on Windows)
                # Keep anything that looks like a real disk
    ...
    def ingest(self):
        ...
        try:
            ...
            for name, c in counters.items():
                ...
                # busy_time is in ms; delta ms busy / delta ms elapsed * 100 = active %
                busy = getattr(c, 'busy_time', 0)
...
class BarWidget:
    H = 14; R = 7
    def __init__(self, parent, width=310, bg=PANEL_BG):
        self.w = width
        self.c = tk.Canvas(parent, width=width, height=self.H,
                           bg=bg, highlightthickness=0)
        self.c.pack(anchor="w", pady=(2,0))
        r,w,h = self.R, self.w, self.H
        self.c.create_arc(0,0,r*2,h,start=90,extent=180,fill=BORDER,outline="")
        self.c.create_arc(w-r*2,0,w,h,start=-90,extent=180,fill=BORDER,outline="")
        self.c.create_rectangle(r,0,w-r,h,fill=BORDER,outline="")

    def set(self, pct, colour):
        self.c.delete("fill")
        fw = max(int(self.w * pct/100), 1)
        r,h = self.R, self.H
        if fw >= r*2:
            self.c.create_arc(0,0,r*2,h,start=90,extent=180,fill=colour,outline="",tags="fill")
            self.c.create_rectangle(r,0,fw,h,fill=colour,outline="",tags="fill")
        else:
            self.c.create_rectangle(0,0,fw,h,fill=colour,outline="",tags="fill")
        if fw >= self.w-r:
            self.c.create_arc(self.w-r*2,0,self.w,h,start=-90,extent=180,
                              fill=colour,outline="",tags="fill")


class SparkChart:
    """Stateless line chart — call draw() with an external data slice each tick."""
    def __init__(self, parent, label, colour, width=310, height=60, bg=PANEL_BG):
        self.colour = colour
        ...
        self.canvas.pack()
        for pct in [25,50,75]:
            y = height - int(height*pct/100)
            self.canvas.create_line(0,y,width,y,fill=BORDER,dash=(2,4))
        for pct in [0,50,100]:
            y = height - int(height*pct/100) - 2
    ...
    def draw(self, data, window_sec, max_val=100):
        ...
        # ── KEY FIX: slice to requested window before drawing ──
        d = list(data[-window_sec:]) if data else []
        if len(d) < 2:
            return
        w, h = self.w, self.h
        step = w / (len(d)-1)
        ...
        self.canvas.create_polygon(flat, fill=lerp_hex(self.colour,"#0a0c10",0.82),
                                   outline="", tags="spark")
        self.canvas.create_line([c for p in pts for c in p], fill=self.colour,
                                width=1.5, smooth=True, tags="spark")
        lx,ly = pts[-1]
        self.canvas.create_oval(lx-3,ly-3,lx+3,ly+3,
                                fill=self.colour,outline=BG,width=1,tags="spark")
...
class ArcGauge:
    def __init__(self, parent, label, unit, colour, size=160):
        ...
        self.val_lbl = tk.Label(parent,textvariable=self.val_var,
                                bg=PANEL_BG,fg=WHITE,font=("Consolas",16,"bold"))
        self.val_lbl.pack()
...
class SystemPanel(tk.Frame):
    ...
    def __init__(self, parent, sys_data, window_sec_ref, show_spark=False):
        ...
        self.sd  = sys_data
        self.win = window_sec_ref   # callable -> int
        ...
        for key, label, unit, col, has_bar in specs:
            ...
            if has_bar:
                ...
                bar = BarWidget(bf, width=bar_w, bg=PANEL_BG)
            self.rows[key] = {"var":vv,"lbl":vl,"bar":bar,"col":col}

        # Spark charts (only in history layout)
        self.sparks = {}
        if show_spark:
            ...
            self.sparks["cpu"]  = SparkChart(sf, "CPU LOAD %", ACCENT_ORG,   width=560, height=45)
            self.sparks["ram"]  = SparkChart(sf, "RAM USED %", ACCENT_PURP,  width=560, height=45)
            if self.sd.cpu_temp is not None or True:
                self.sparks["ctemp"] = SparkChart(sf, "CPU TEMP °C", ACCENT_GREEN, width=560, height=45)
    ...
    def refresh(self):
        ...
        if self.sparks:
            ...
            if "ctemp" in self.sparks:
                self.sparks["ctemp"].draw(sd.h_ctemp, w, 120)


# ══════════════════════════════════════════════════════════════════════════════
#  Disk I/O panel (shared across all layouts)
# ══════════════════════════════════════════════════════════════════════════════

class DiskPanel(tk.Frame):
    """Per-disk transfer rate + active time panel, toggleable in every layout."""
    # Dynamic max for transfer rate bars — scales up, slow decay
    _rate_max = 100.0   # MB/s ceiling, shared across instances via class var
    ...
    def __init__(self, parent, disk_data, window_sec_ref, show_spark=False):
        ...
        self.show_spark = show_spark
        self._disk_widgets = {}  # disk_name -> row widgets
        self._disk_sparks  = {}  # disk_name -> spark dict
    ...
    def _build_disk_rows(self):
        """Build one block of rows per disk. Called once; refreshes update values."""
        ...
        if not names:
            tk.Label(self.body, text="No disk I/O data available",
                     bg=PANEL_BG, fg=TEXT_DIM, font=("Consolas",8)).pack()
            return
        ...
        for disk in names:
            ...
            # Sparks (Layout 2 only)
            if self.show_spark:
                tk.Frame(self.body, bg=BORDER, height=1).pack(fill="x", pady=(4,4))
                sf = tk.Frame(self.body, bg=PANEL_BG)
                sf.pack(fill="x")
                self._disk_sparks[disk] = {
                    "read":   SparkChart(sf, f"{disk} READ MB/s",   ACCENT_CYAN,  width=560, height=40),
                    "write":  SparkChart(sf, f"{disk} WRITE MB/s",  ACCENT_PURP,  width=560, height=40),
                    "active": SparkChart(sf, f"{disk} ACTIVE %",    ACCENT_TEAL,  width=560, height=40),
                }
    ...
    def refresh(self):
        # Rebuild rows if disk list changed (e.g. USB inserted)
        if set(self.dd.disk_names) != set(self._disk_widgets.keys()):
            self._build_disk_rows()

        # Update dynamic rate ceiling (peak * 1.2, minimum 10 MB/s)
        all_rates = []
        ...
        if all_rates:
            ...
            if peak > DiskPanel._rate_max:
                ...
                # Slow decay toward actual peak
                DiskPanel._rate_max = max(DiskPanel._rate_max * 0.995, max(peak * 1.2, 10.0))
...
class Layout1(tk.Frame):
    def __init__(self, parent, gpu_data_list, sys_data, disk_data):
        ...
        tog_sys = make_toggle(tb, "⊞  SYSTEM RESOURCES", "⊟  SYSTEM RESOURCES", self._toggle_sys)
        ...
        for gd in gpu_data_list:
            ...
            p.frame.grid(row=0, column=gd.index, padx=8, pady=4, sticky="n")
            self.panels.append(p)

        self.sys_holder  = tk.Frame(self, bg=BG)
        self.sys_holder.pack(fill="x", padx=16, pady=(0,4))
        self.disk_holder = tk.Frame(self, bg=BG)
        self.disk_holder.pack(fill="x", padx=16, pady=(0,8))

    def _toggle_sys(self, on):
        for w in self.sys_holder.winfo_children(): w.destroy()
        self.sys_panel = None
        if on:
            self.sys_panel = SystemPanel(self.sys_holder, self.sd,
                                         lambda: DEFAULT_SPAN, show_spark=False)
            self.sys_panel.pack(fill="x", padx=8, pady=4)
            self.sys_panel.refresh()

    def _toggle_disk(self, on):
        for w in self.disk_holder.winfo_children(): w.destroy()
        self.disk_panel = None
        if on:
            self.disk_panel = DiskPanel(self.disk_holder, self.dd,
                                        lambda: DEFAULT_SPAN, show_spark=False)
            self.disk_panel.pack(fill="x", padx=8, pady=4)
    ...
    def _make_panel(self, parent, gd):
        ...
        for key, label, unit, has_bar in [
            ...
            bar=None
            if has_bar:
                bf=tk.Frame(mf,bg=PANEL_BG); bf.pack(fill="x",pady=(0,2))
                bar=BarWidget(bf,width=310)
    ...
    def refresh(self):
        ...
        if self.sys_panel:  self.sys_panel.refresh()
        if self.disk_panel: self.disk_panel.refresh()
...
class Layout2(tk.Frame):
    def __init__(self, parent, gpu_data_list, sys_data, disk_data):
        ...
        self.disk_panel = None
        self.panels = []
        # window_sec is an int held here; pass a lambda to SparkChart
    ...
    def _make_panel(self, parent, gd):
        ...
        sparks = {
            "load": SparkChart(sf, "LOAD %",  ACCENT_CYAN,  width=300, height=50),
            "vram": SparkChart(sf, "VRAM %",  ACCENT_PURP,  width=300, height=50),
            "temp": SparkChart(sf, "TEMP °C", ACCENT_GREEN, width=300, height=50),
        }
        ...
        for key, label, unit, has_bar in [
            ...
            r = tk.Frame(mf, bg=PANEL_BG); r.pack(fill="x", pady=(0,8))
            tk.Label(r,text=label,bg=PANEL_BG,fg=TEXT_DIM,
                     font=("Consolas",8),width=5,anchor="w").pack(side="left")
            vv = tk.StringVar(value="—")
            vl = tk.Label(r,textvariable=vv,bg=PANEL_BG,fg=WHITE,
    ...
    def refresh(self):
        ...
        for p in self.panels:
            ...
            p["rows"]["temp"]["var"].set(str(gd.temp))
            p["rows"]["temp"]["lbl"].config(fg=ct)
            # ── Draw sparks from GPUData history, sliced to current window ──
    ...
    def __init__(self, parent, gpu_data_list, sys_data, disk_data):
        ...
        for gd in gpu_data_list:
            ...
            for key,col in [("LOAD",ACCENT_CYAN),("VRAM",ACCENT_PURP),("TEMP",ACCENT_GREEN)]:
                ...
                items[key]=vv
            dot=tk.Label(f,text="●",bg=PANEL_BG,fg=ACCENT_GREEN,font=("Consolas",9))
            dot.pack()
        ...
        for ri, (label, unit, key, get_val, to_pct) in enumerate(self.METRICS):
            ...
            for gd in gpu_data_list:
                ...
                self.cells[(key, gd.index)] = {
                    ...
                    "get_val":get_val,"to_pct":to_pct,"hist_key":key,"gd":gd,
                }
...
class Layout5(tk.Frame):
    ...
    def _make_panel(self, gd):
        ...
        tk.Label(hdr,text=gd.name,bg=PANEL_BG,fg=TEXT_DIM,
                 font=("Consolas",8)).pack(side="left",padx=(8,0))
        dot=tk.Label(hdr,text="●",bg=PANEL_BG,fg=ACCENT_GREEN,
                     font=("Consolas",10)); dot.pack(side="right")
        tk.Frame(f,bg=BORDER,height=1).pack(fill="x",padx=16,pady=(0,8))
        metric_row=tk.Frame(f,bg=PANEL_BG); metric_row.pack(fill="x",padx=20,pady=(0,14))
        entries={}
        for ci,(label,unit,col,key) in enumerate([
            ...
            box=tk.Frame(metric_row,bg=PANEL_BG); box.grid(row=0,column=ci,padx=20)
            tk.Label(box,text=label,bg=PANEL_BG,fg=TEXT_DIM,
                     font=("Consolas",8)).pack(anchor="w")
            vv=tk.StringVar(value="—")
            vl=tk.Label(box,textvariable=vv,bg=PANEL_BG,fg=col,
                        font=("Consolas",34,"bold"),anchor="w"); vl.pack(anchor="w")
            ...
            tl=tk.Label(box,textvariable=tv,bg=PANEL_BG,fg=TEXT_DIM,
                        font=("Consolas",9)); tl.pack(anchor="w")
    ...
    def _build_chrome(self):
        ...
        for gd in self.gpu_data:
            tk.Label(name_frame,
                     ...
                     bg=BG, fg=TEXT_DIM, font=("Consolas",7)).pack(side="left", padx=(0,16))
